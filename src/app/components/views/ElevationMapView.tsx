...
export function ElevationMapView() {
  ...
  const { data: elevationGrid, loading, error, fetchElevations, fetchSingleElevation } =
    useElevation();
  ...
  useEffect(() => {
    async function loadAnimalElevations() {
      ...
      for (const animal of animals) {
        const elev = await fetchSingleElevation(animal.lat, animal.lng);
        elevs[animal.id] = elev;
      }
      setAnimalElevations(elevs);
    }
    if (animals.length > 0) loadAnimalElevations();
  }, [fetchSingleElevation, animals]);
