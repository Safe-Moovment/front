...
export function MapView() {
  ...
  const activeFence = fences.find(f => f.status === "active") || fences[0];
  const svgFencePoints = activeFence.coordinates
    .map(([lat, lng]) => {
  ...
  return (
    <div className="relative w-full h-full rounded overflow-hidden border border-[#4A5A4D]">
      ...
      <iframe
        ...
        src={`https://www.google.com/maps/embed/v1/view?key=AIzaSyBFw0Qbyq9zTFTd-tUY6dZWTgaQzuU17R8&center=${center.lat},${center.lng}&zoom=${zoom}&maptype=${mapType === 'terrain' ? 'roadmap' : 'satellite'}`}
      />
      ...
      <div className="absolute top-2 md:top-4 left-2 md:left-4 bg-white/95 px-2 md:px-3 py-1.5 md:py-2 rounded shadow-md border border-[#E5E5E5]">
        ...
        <p className="font-semibold text-sm md:text-base text-[#2C2C2C]">{animals.length} animales</p>
